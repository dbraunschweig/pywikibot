...
def delete_broken_redirects():
    print("Deleting Broken Redirects:")
    regex = re.compile("#REDIRECT \[\[[^\]]*\]\]", re.IGNORECASE)
...
def delete_files_with_no_source():
    pages = categorymembers("Files_with_no_machine-readable_source")
    for page in pages:

        print(page.title())
        page.delete(reason="Missing license and/or source information", prompt=False)


def delete_files_with_no_license():
    page = pywikibot.Page(site, "Template:No license")
    pages = page.embeddedin()
...
def fix_file_summary():

    pages = site.allpages(namespace=6, content=True, prefix="V", start="V")
    save = True
    for page in pages:
        title = page.title()
        text = page.text
...
def show_non_empty_category_redirects():
    pages = embeddedin("Template:Category redirect", namespaces=14)
    for page in pages:
        title = page.title()
        members = categorymembers(title)
        for member in members:
            print(title)
            break


def change_category_template():
    page = pywikibot.Page(site, "Film school:Completed Homework Assignments")
    text = page.text
    matches = re.findall("\{\{.*\}\}", text)
    for match in matches:
        match = "Template:" + match[2:-2]
        page = pywikibot.Page(site, match)
        text = page.text
        if text.find("[[Category:Homework Assignment]]") >= 0:
            print(page.title())
            text = text.replace("[[Category:Homework Assignment]]", "[[Category:Homework assignments]]")
            page.text = text
            page.save(summary="Category", minor=True, botflag=True)


def move_topic():
    pages = site.allpages(prefix="Accounting", namespace=104, filterredir=False)
    for page in pages:
        title = page.title()
        print(title)

        title = title.replace("Topic:", "Portal:")
    ...
    for match in re.finditer("\[\[([^\]]*)\]\]", text):
        ...
        if link.isRedirectPage():
            ...
            if link.title() == title:
                ...
                item = match.start(1), link.title() + "|" + title, match.end(1)
            list.append(item)
...
def show_unused_redirects(start):
    ...
    for page in pages:
        ...
        found = False
        for link in links:
            if link.title() == "Wikiversity:Orphaned Redirects":
                continue
            found = True
            break
...
# print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
# show_unused_redirects("A")

#show_non_empty_category_redirects()
